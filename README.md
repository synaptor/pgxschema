...
`PGXSchema` is a Go library for managing Postgres database schemas with a focus on rollout safety and backward and forward data compatibility.
...
- Schema changes that are not safe (i.e., changes that can cause data loss) are logged as warnings for manual execution.

## Usage

Call `pgxschema.Sync` directly from your application startup code:
...
Designing safe data migrations in distributed systems is a complex topic. For reliability reasons, upgrading the entire
application to a new version in a lockstep is often not possible or desirable. Typically, operators want to perform a
...
(i.e., the old version of the application did not write any value to it), but the new version must not assume that all data readers
will know about the new column.

Removing a column, by contrast, is not safe because old versions of the application might try to access the removed column,
...
Let's consider a practical example: changing a column `hostport`, which contains a host:port pair as a string, to two separate columns
`host` and `port`:

- We add new columns `host` and `port` with default values of NULL. This is a safe operation. Nobody is using these columns yet.
- We deploy the new version of the application that writes to `hostport` as well as the new `host` and `port` columns.
- Old versions of the application continue to read from and write to `hostport`.
- New versions of the application write to `hostport` as well as the new `host` and `port` columns.
- New versions of the application read from `host` and `port`, and if they are NULL, they fall back to parsing `hostport`.
- Once old versions of the application are no longer running and rolling back to the old version is no longer possible, we stop
...
the operator is certain that rolling back to the old version of the application is not possible.
...
to create them automatically but to drop them manually. This avoids index flapping during progressive rollouts, when different
...
It might be surprising, but yes. There is no way to reliably detect when it is safe to perform an unsafe operation automatically. Most ORM frameworks that facilitate schema migrations assume that the application is deployed in a lockstep, and do not support progressive
